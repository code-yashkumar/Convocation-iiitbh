...
export function AccommodationCTA({ className = '' }) {
  return (
    <div
      className={`w-full max-w-[87.5rem] mx-auto px-4 sm:px-10 lg:px-12 my-5 sm:my-8 ${className}`}
    >
      <div className="relative bg-gradient-to-r from-[#500D16] via-[#66101E] to-[#450A12] text-white rounded-[24px] sm:rounded-[32px] p-6 sm:p-10 lg:p-12 overflow-hidden shadow-sm border border-maroon-800/80 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 sm:gap-8">
        ...
        <div className="relative z-10 max-w-2xl space-y-2.5 sm:space-y-3">
          <div className="inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-3.5 py-1 rounded-full bg-white/15 backdrop-blur-md border border-white/20 text-gold-300 font-body text-[11px] sm:text-xs font-semibold uppercase tracking-wider whitespace-nowrap">
        ...
        <div className="relative z-10 shrink-0 w-full sm:w-auto">
          <Link
            to="/accommodation"
            className="group inline-flex items-center justify-center min-h-[48px] sm:min-h-[50px] px-5 sm:px-8 rounded-pill bg-gold-500 text-charcoal-950 font-body font-bold text-[13px] min-[360px]:text-[13.5px] sm:text-[15px] shadow-lg hover:bg-gold-400 active:scale-95 transition-all gap-2 sm:gap-3 w-full sm:w-auto whitespace-nowrap text-center"
          >
